...
export class CoreConfigService {
  ...
  public mainLogo = 'assets/images/logo/main-logo.png'
  ...
  constructor(private _router: Router, @Inject(CORE_CUSTOM_CONFIG) private _config) {
    ...
    if (_config.layout.enableLocalStorage) {
      this.localConfig = JSON.parse(localStorage.getItem('surveyapp-config'));
    } else {
      localStorage.removeItem('surveyapp-config');
    }
  ...
  set config(data) {
    ...
    if (config.layout.enableLocalStorage) {
      localStorage.setItem('surveyapp-config', JSON.stringify(config));
    }
