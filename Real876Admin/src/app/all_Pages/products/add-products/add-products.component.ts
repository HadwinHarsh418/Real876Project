...
export class AddProductsComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder,public toaster:ToastrManager,private modalService:NgbModal,private ActiveModal:NgbActiveModal,
    private apiService:ProductService, private router:Router,private autheticationService:AuthenticationService ) {
      this.autheticationService.currentUser.subscribe((res:any)=>{
  ...
  updateValidation(){
    Object.keys(this.ProductForm.controls).forEach(it=>{
      if(this.disabledFieldArray.includes(it)){
        if(it == 'about_the_company_check'){
        this.validators(this.getProductItem[it],it.split('_c')[0],'about_the_company_check')
        }else{
          this.validators(this.getProductItem[it],it.split('_c')[0])
        }
      }
    })
  ...
  validators(value: any, control: any,data?) {
    if (value == 'Yes' || value == '') {
      switch (control) {
        case 'product_name': {
